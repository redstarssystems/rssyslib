...
- project control via `Justfile` (see {just});
...
Creating a project from this template (the `:clj-new` alias in this template project defaults `:template` to rssyslib):
...
See `README.adoc` in a root folder of created project.
